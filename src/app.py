...
def short_term_analysis(data, stock_symbol):
    ...
    # Example: Predict next day's price using a simple linear regression model
...
def long_term_analysis(ticker):
    ...
    data = yf.download(ticker, period='5y')  # Downloading 5 years of data
    data['Price'] = data['Close']

    # Ensure index is a datetime format
    data.index = pd.to_datetime(data.index)

    # Differencing the data to make it stationary for ARIMA
    data['Price_diff'] = data['Price'].diff().dropna()

    # ARIMA model (p=5, d=1, q=0) as an example, you can tweak parameters
    model = ARIMA(data['Price_diff'].dropna(), order=(5, 1, 0))
    model_fit = model.fit()

    # Predict future values, e.g., 30 days ahead
    forecast = model_fit.forecast(steps=30)
    last_price = data['Price'].iloc[-1]
    predicted_prices = last_price + forecast.cumsum()
    return predicted_prices.iloc[-1]  # Return the predicted price 30 days from today
...
def predict_stock(stock_symbol, term):
    ...
    try:
        ...
        if len(data) < 2:  # Check if there is enough data
